...
def train_and_evaluate_cnn(X_train_raw, X_test_raw, y_train_raw, y_test_raw):
    X_train, X_test, y_train, y_test = preprocess_for_cnn(X_train_raw, X_test_raw, y_train_raw, y_test_raw)
    ...
    for n in range(n_epochs):
        ...
        for i, (X, y) in enumerate(train_dataloader):
            ...
            epoch_acc += (pred.argmax(dim=1) == y).sum().item()
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for i, (X, y) in enumerate(test_dataloader):
                ...
                test_acc += (pred.argmax(dim=1) == y).sum().item()
